...
function createModal(initialAbsences = [], initialLates = []) {
  ...
  const absenceSection = createSection(
    "Absences",
    "absenceDate",
    "absenceList",
    "submitAbsence",
  ...
  const lateSection = createSection(
    "Lates",
    "lateDate",
    "lateList",
    "submitLate",
...
function createInitialDateItem(date, listId) {
  ...
  deleteButton.addEventListener("click", () =>
    deleteDate(dateItem, listId, date)
  );
...
${instructor}
Or 
${support} with the Learner Support Team`;
        console.log(str);
    ...
    late.addEventListener("click", (event) => {
      ...
      chrome.storage.sync.get([name, "settings"], (res) => {
        ...
        let str = `Hello ${name.split(" ")[0]}, 
This email is to let you know that you were recorded late to class on ${absences[0].formatedDate}.
This is your ${occurences} tardie.
Per your enrollment agreement, you only have ${left} tardie${ left > 1 ? "s" : ""} left at your disposal. 
